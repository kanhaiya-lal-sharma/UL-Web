...
test.skip("Join waitlist form",async({page})=>{

    await page.goto("https://ul1.devbeta.in/united-kingdom/london/property/iq-sterling-court");

  // Click first Join waitlist  under Bronze En Suite
  const firstJoinWaitlist = page.locator('div')
    .filter({ hasText: 'Bronze En Suite' })
    .getByRole('button', { name: /Join waitlist/i })
    .first();

  await firstJoinWaitlist.click();
  ...
  await expect(page).toHaveURL("https://ul1.devbeta.in/united-kingdom/london/iq-sterling-court/wait-list/thank-you");
